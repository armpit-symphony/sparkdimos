...
from lima.models.base import HuggingFaceModel, HuggingFaceModelConfig
...
class MoondreamVlModel(HuggingFaceModel, VlModel):
    ...
    def _to_pil(self, image: Image | np.ndarray[Any, Any]) -> PILImage.Image:
        """Convert lima Image or numpy array to PIL Image, applying auto_resize."""
        if isinstance(image, np.ndarray):
            warnings.warn(
                "MoondreamVlModel should receive standard lima Image type, not a numpy array",
                DeprecationWarning,
                stacklevel=2,
            )
