...
from lima.perception.experimental.temporal_memory import (
    Frame,
...
from lima.perception.experimental.temporal_memory.entity_graph_db import EntityGraphDB
from lima.perception.experimental.temporal_memory.temporal_utils.graph_utils import (
    extract_time_window,
)
...
class TestPersistence:
    def test_new_memory_clears_db(self, tmp_path: Path) -> None:
        ...
        with patch(
            "lima.perception.experimental.temporal_memory.temporal_memory.get_run_log_dir",
            return_value=None,
        ):
            tm = TemporalMemory(
                vlm=MagicMock(),
...
class TestEntityMarkers:
    ...
    def test_markers_to_rerun(self) -> None:
        from lima.msgs.visualization_msgs.EntityMarkers import EntityMarkers, Marker
...
class TestWindowAnalyzer:
    def test_analyze_window_calls_vlm(self) -> None:
        from lima.perception.experimental.temporal_memory.window_analyzer import WindowAnalyzer
...
class TestTemporalMemoryIntegration:
    ...
    @pytest.fixture(scope="function")
    def video_module(self, lima_cluster):
        video_module = lima_cluster.deploy(VideoReplayModule, num_frames=8)
    ...
    @pytest.fixture(scope="function")
    def temporal_memory_module(self, lima_cluster, tmp_path):
        from lima.models.vl.openai import OpenAIVlModel
    ...
    def test_frames_flow_and_query(
        self, lima_cluster, video_module, temporal_memory_module, tmp_path
    ):
