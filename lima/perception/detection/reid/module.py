...
from lima_lcm.foxglove_msgs.Point2 import Point2
...
from lima.msgs.foxglove_msgs.Color import Color
...
from lima.perception.detection.reid.type import IDSystem
from lima.perception.detection.type.detection2d.imageDetections2D import ImageDetections2D
from lima.types.timestamped import align_timestamped, to_ros_stamp
...
class ReidModule(Module):
    ...
    def __init__(self, idsystem: IDSystem | None = None, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        if idsystem is None:
            try:
                from lima.models.embedding import TorchReIDModel

                idsystem = EmbeddingIDSystem(model=TorchReIDModel, padding=0)  # type: ignore[arg-type]
            except Exception as e:
                raise RuntimeError(
                    "TorchReIDModel not available. Please install with: pip install lima[torchreid]"
                ) from e
