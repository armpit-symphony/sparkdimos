...
class LCMTopicProto(Protocol):
    ...
    lcm_type: type[LimaMsg] | None


class LCMEncoderMixin(PubSubEncoderMixin[LCMTopicProto, LimaMsg, bytes]):
    """Encoder mixin for LimaMsg using LCM binary encoding."""

    def encode(self, msg: LimaMsg | bytes, _: LCMTopicProto) -> bytes:
        if isinstance(msg, bytes):
            return msg
        return msg.lcm_encode()

    def decode(self, msg: bytes, topic: LCMTopicProto) -> LimaMsg:
...
class JpegEncoderMixin(PubSubEncoderMixin[LCMTopicProto, Image, bytes]):
    """Encoder mixin for LimaMsg using JPEG encoding (for images)."""
