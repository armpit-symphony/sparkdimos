...
from lima.msgs.sensor_msgs.Image import Image, ImageFormat
from lima.protocol.pubsub.benchmark.type import Case
...
try:
    ...
    DDS_AVAILABLE = False
from lima.protocol.pubsub.impl.lcmpubsub import LCM, LCMPubSubBase, Topic as LCMTopic
from lima.protocol.pubsub.impl.memory import Memory
from lima.protocol.pubsub.impl.shmpubsub import (
    BytesSharedMemory,
    LCMSharedMemory,
    PickleSharedMemory,
)
...
if DDS_AVAILABLE:
    ...
    from lima.protocol.pubsub.impl.ddspubsub import (
        DDS,
        Topic as DDSTopic,
    )
...
try:
    from lima.protocol.pubsub.impl.redispubsub import Redis
...
from lima.protocol.pubsub.impl.rospubsub import (
    ROS_AVAILABLE,
    limaROS,
    RawROS,
    RawROSTopic,
...
if ROS_AVAILABLE:
    ...
    @contextmanager
    def lima_ros_best_effort_pubsub_channel() -> Generator[limaROS, None, None]:
        qos = QoSProfile(  # type: ignore[no-untyped-call]
            reliability=QoSReliabilityPolicy.BEST_EFFORT,
            history=QoSHistoryPolicy.KEEP_LAST,
            durability=QoSDurabilityPolicy.VOLATILE,
        ...
        ros_pubsub = limaROS(node_name="benchmark_lima_ros_best_effort", qos=qos)
        ros_pubsub.start()
        yield ros_pubsub
        ros_pubsub.stop()

    @contextmanager
    def lima_ros_reliable_pubsub_channel() -> Generator[limaROS, None, None]:
        qos = QoSProfile(  # type: ignore[no-untyped-call]
            reliability=QoSReliabilityPolicy.RELIABLE,
            history=QoSHistoryPolicy.KEEP_LAST,
            durability=QoSDurabilityPolicy.VOLATILE,
        ...
        ros_pubsub = limaROS(node_name="benchmark_lima_ros_reliable", qos=qos)
        ros_pubsub.start()
        yield ros_pubsub
        ros_pubsub.stop()

    def lima_ros_msggen(size: int) -> tuple[ROSTopic, Image]:
        topic = ROSTopic(topic="/benchmark/lima_ros", msg_type=Image)
        return (topic, make_data_image(size))
    ...
    # testcases.append(
    #     Case(
    #         pubsub_context=lima_ros_best_effort_pubsub_channel,
    #         msg_gen=lima_ros_msggen,
    #     )
    # )

    testcases.append(
        Case(
            pubsub_context=lima_ros_reliable_pubsub_channel,
            msg_gen=lima_ros_msggen,
        )
    )
