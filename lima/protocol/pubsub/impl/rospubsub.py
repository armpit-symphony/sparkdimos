...
from lima.protocol.pubsub.impl.rospubsub_conversion import (
    derive_ros_type,
    lima_to_ros,
    ros_to_lima,
)
...
@dataclass
class ROSTopic:
    """Topic descriptor for limaROS pubsub (uses lima message types)."""

    topic: str
    msg_type: type[LimaMsg]
    qos: "QoSProfile | None" = None
...
class RawROS(PubSub[RawROSTopic, Any]):
    ...
    def __init__(self, node_name: str | None = None, qos: "QoSProfile | None" = None) -> None:
        ...
        self._node_name = node_name or f"lima_ros_{uuid.uuid4().hex[:8]}"
        ...
        if qos is not None:
            ...
            self._qos = QoSProfile(  # type: ignore[no-untyped-call]
                ...
                # ./bin/dev python -m pytest -svm tool -k ros lima/protocol/pubsub/benchmark/test_benchmark.py
                #
...
class limaROS(PubSub[ROSTopic, LimaMsg]):
    """ROS PubSub with automatic lima.msgs ↔ ROS message conversion.

    Uses ROSTopic (with lima msg_type) instead of RawROSTopic (with ros_type).
    Automatically converts between lima and ROS message formats.
    Uses composition with RawROS internally.
    """

    def __init__(self, node_name: str | None = None, qos: "QoSProfile | None" = None) -> None:
        """Initialize the limaROS pubsub.
    ...
    def publish(self, topic: ROSTopic, message: LimaMsg) -> None:
        """Publish a lima message to a ROS topic.

        Args:
            topic: ROSTopic with lima msg_type
            message: LIMA Robotics message to publish
        """
        raw_topic = self._to_raw_topic(topic)
        ros_message = lima_to_ros(message, raw_topic.ros_type)
        self._raw.publish(raw_topic, ros_message)

    def subscribe(
        self, topic: ROSTopic, callback: Callable[[LimaMsg, ROSTopic], None]
    ) -> Callable[[], None]:
        """Subscribe to a ROS topic with automatic lima message conversion.

        Args:
            topic: ROSTopic with lima msg_type
            callback: Function called with (lima_message, topic)
        ...
        def wrapped_callback(ros_msg: Any, _raw_topic: RawROSTopic) -> None:
            lima_msg = ros_to_lima(ros_msg, topic.msg_type)
            callback(lima_msg, topic)
