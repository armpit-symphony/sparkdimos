...
from lima_lcm.geometry_msgs import PointStamped
import numpy as np
import pytest
...
@pytest.fixture()
def publisher() -> Generator[limaROS, None, None]:
    yield from ros_node()


@pytest.fixture()
def subscriber() -> Generator[limaROS, None, None]:
    yield from ros_node()
...
def test_basic_conversion(publisher, subscriber):
    ...
    Simple flat lima.msgs type with no nesting (just x/y/z floats).
    """
...
def test_posestamped_pubsub(publisher, subscriber):
    ...
    COMPLEX_TYPE with custom lima.msgs implementation and nested messages
...
def test_pointstamped_pubsub(publisher, subscriber):
    ...
    Raw LCM type with nested messages (Header, Point) but NO custom lima.msgs
...
def test_twist_pubsub(publisher, subscriber):
    ...
    lima.msgs type with nested Vector3 messages (linear, angular).
    Tests recursive field copy with custom lima.msgs nested types.
    """
    original = Twist(
        linear=[1.0, 2.0, 3.0],
        angular=[0.1, 0.2, 0.3],
