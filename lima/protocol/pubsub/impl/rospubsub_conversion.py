...
"""Conversion functions between lima messages and ROS messages.

This module provides conversion functions between lima message types and ROS messages.
...
3. No lima.msgs equivalent - return lima_lcm type
"""
...
if TYPE_CHECKING:
    from lima.msgs import LimaMsg
    from lima.protocol.pubsub.impl.rospubsub import ROSMessage


# Complex types that need LCM roundtrip (explicit list)
# These types have different internal representations in dima vs ROS/LCM
...
# Cache for dynamic imports of lima types
_lima_type_cache: dict[str, type[LimaMsg] | None] = {}

# Cache for LCM type derivation
_lcm_type_cache: dict[str, type[Any]] = {}

# Field name mappings between ROS and LCM (ROS name -> LCM name)
# This is some mixup in lima_lcm having ROS1 and ROS2 message definitions?
...
def get_lima_type(msg_name: str) -> type[LimaMsg] | None:
    """Try to import lima.msgs type, return None if not found. Cached.

    Args:
        msg_name: Message name in format "package.MessageName" (e.g., "geometry_msgs.Vector3")

    Returns:
        The lima message type, or None if not found
    """
    if msg_name in _lima_type_cache:
        return _lima_type_cache[msg_name]

    try:
        package, name = msg_name.split(".")
        module = importlib.import_module(f"lima.msgs.{package}.{name}")
        lima_type = cast("type[LimaMsg]", getattr(module, name))
        _lima_type_cache[msg_name] = lima_type
        return lima_type
    except (ImportError, AttributeError, ValueError):
        _lima_type_cache[msg_name] = None
        return None


def derive_lcm_type(lima_type: type[LimaMsg]) -> type[Any]:
    """Derive the LCM message type from a lima message type.

    Args:
        lima_type: A lima message type (e.g., lima.msgs.sensor_msgs.PointCloud2)

    Returns:
        The corresponding LCM message type (e.g., lima_lcm.sensor_msgs.PointCloud2)
    """
    msg_name = lima_type.msg_name  # e.g., "sensor_msgs.PointCloud2"
    ...
    lcm_module = importlib.import_module(f"lima_lcm.{package}.{message_name}")
    lcm_type: type[Any] = getattr(lcm_module, message_name)
    _lcm_type_cache[msg_name] = lcm_type
    return lcm_type


def derive_ros_type(lima_type: type[LimaMsg]) -> type[ROSMessage]:
    """Derive the ROS message type from a lima message type.

    Args:
        lima_type: A lima message type (e.g., lima.msgs.geometry_msgs.Vector3)
    ...
    """
    msg_name = lima_type.msg_name  # e.g., "geometry_msgs.Vector3"
    parts = msg_name.split(".")
    if len(parts) != 2:
        raise ValueError(f"Invalid msg_name format: {msg_name}, expected 'package.MessageName'")
...
def _create_lcm_instance_for_ros_msg(ros_msg: Any) -> Any:
    ...
    # Convert to LCM module path (std_msgs.msg.Header -> lima_lcm.std_msgs.Header)
    package = module_name.split(".")[0]  # e.g., "std_msgs"
    lcm_module = importlib.import_module(f"lima_lcm.{package}.{class_name}")
...
def _create_ros_instance_for_lcm_msg(lcm_msg: Any, ros_type_hint: str) -> Any:
    ...
    module_name = lcm_type.__module__  # e.g., "lima_lcm.std_msgs.Header"
...
def lima_to_ros(msg: LimaMsg, ros_type: type[ROSMessage]) -> ROSMessage:
    """Convert a lima message to a ROS message.
    ...
    Args:
        msg: LIMA Robotics message instance
    ...
    if msg_name in COMPLEX_TYPES:
        # Complex: dima → encode → decode LCM → copy to ROS
...
def ros_to_lima(msg: Any, lima_type: type[LimaMsg]) -> LimaMsg:
    """Convert a ROS message to a lima message.

    For complex types (PointCloud2, Image, CameraInfo), uses LCM roundtrip
    to properly build the lima internal representation. For simple types,
    uses direct field copy.

    Args:
        msg: ROS message instance
        lima_type: Target lima message type

    Returns:
        LIMA Robotics message instance
    """
    msg_name = lima_type.msg_name

    if msg_name in COMPLEX_TYPES:
        # Complex: ROS → LCM → encode → decode → lima
        lcm_type = derive_lcm_type(lima_type)
        lcm_msg = lcm_type()
        _copy_ros_to_lcm_recursive(msg, lcm_msg)
        return lima_type.lcm_decode(lcm_msg.lcm_encode())

    # Simple type: recursive field copy (handles nested messages)
    lima_msg = lima_type()
    _copy_ros_to_lcm_recursive(msg, lima_msg)
    return lima_msg
