...
from lima.protocol.pubsub.impl.lcmpubsub import Topic
from lima.protocol.service.lcmservice import (
    _DEFAULT_LCM_URL,
    LCMConfig,
    LCMService,
    autoconf,
)
from lima.protocol.service.system_configurator import (
...
class TestConfigureSystemForLcm:
    def test_creates_linux_checks_on_linux(self) -> None:
        with patch(
            "lima.protocol.service.system_configurator.platform.system", return_value="Linux"
        ):
            with patch("lima.protocol.service.lcmservice.configure_system") as mock_configure:
                autoconf()
    ...
    def test_creates_macos_checks_on_darwin(self) -> None:
        with patch(
            "lima.protocol.service.system_configurator.platform.system", return_value="Darwin"
        ):
            with patch("lima.protocol.service.lcmservice.configure_system") as mock_configure:
                autoconf()
                mock_configure.assert_called_once()
    ...
    def test_logs_error_on_unsupported_system(self) -> None:
        with patch(
            "lima.protocol.service.system_configurator.platform.system", return_value="Windows"
        ):
            with patch("lima.protocol.service.lcmservice.configure_system") as mock_configure:
                with patch("lima.protocol.service.lcmservice.logger") as mock_logger:
                    autoconf()
...
class TestLCMService:
    def test_init_with_default_config(self) -> None:
        with patch("lima.protocol.service.lcmservice.lcm.LCM") as mock_lcm_class:
