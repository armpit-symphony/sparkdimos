...
from lima.core.transport import pLCMTransport
from lima.e2e_tests.conf_types import StartPersonTrack
from lima.e2e_tests.lima_cli_call import LimaCliCall
from lima.e2e_tests.lcm_spy import LcmSpy
from lima.msgs.geometry_msgs import PoseStamped, Quaternion
from lima.msgs.geometry_msgs.Vector3 import make_vector3
from lima.msgs.std_msgs.Bool import Bool
from lima.simulation.mujoco.person_on_track import PersonTrackPublisher
...
@pytest.fixture
def start_blueprint() -> Iterator[Callable[[str], LimaCliCall]]:
    lima_robot_call = LimaCliCall()

    def set_name_and_start(*demo_args: str) -> LimaCliCall:
        lima_robot_call.demo_args = list(demo_args)
        lima_robot_call.start()
        return lima_robot_call

    yield set_name_and_start

    lima_robot_call.stop()
