...
"""Tests for lima-viewer integration with RerunBridgeModule.

These tests verify that:
1. The lima-viewer binary is installed and discoverable
2. rerun_bindings.spawn() accepts the executable_name parameter
3. bridge.py has the correct spawn logic
4. Version compatibility between rerun-sdk and lima-viewer

These run in CI where lima-viewer is a core dependency, so the binary
...
class TestViewerBinaryInstallation:
    """Verify lima-viewer binary is installed and functional."""

    def test_binary_on_path(self):
        """lima-viewer binary must be discoverable on PATH."""
        path = shutil.which("lima-viewer")
        assert path is not None, (
            "lima-viewer binary not found on PATH. "
            "Ensure 'lima-viewer' is in pyproject.toml dependencies."
        )

    def test_binary_executable(self):
        """lima-viewer binary must be executable."""
        import os

        path = shutil.which("lima-viewer")
        assert path is not None
        assert os.access(path, os.X_OK), f"lima-viewer at {path} is not executable"
...
class TestRerunBindingsInterface:
    ...
    def test_spawn_accepts_executable_name(self):
        ...
        This is the mechanism we use to launch lima-viewer instead of
        ...
        assert "executable_name" in sig.parameters, (
            ...
            "The lima-viewer integration in bridge.py will fail."
        )
...
class TestBridgeSpawnLogic:
    """Verify bridge.py has the correct lima-viewer spawn logic."""

    def test_bridge_references_lima_viewer(self):
        """bridge.py must attempt to spawn lima-viewer."""
        from lima.visualization.rerun.bridge import RerunBridgeModule

        src = inspect.getsource(RerunBridgeModule.start)
        assert "lima-viewer" in src, (
            "bridge.py start() does not reference 'lima-viewer'. "
    ...
    def test_bridge_has_fallback(self):
        """bridge.py must fall back to stock rerun if lima-viewer unavailable."""
        from lima.visualization.rerun.bridge import RerunBridgeModule

        src = inspect.getsource(RerunBridgeModule.start)
        assert "ImportError" in src or "except" in src, (
            "bridge.py start() has no fallback for missing lima-viewer. "
            "Users without lima-viewer will crash."
        )
...
class TestVersionCompatibility:
    """Catch rerun-sdk / lima-viewer version drift before it bites us at runtime."""

    def test_versions_within_one_minor(self):
        """rerun-sdk and lima-viewer must be within 1 minor version.

        lima-viewer is built from a rerun fork, so they track the same
        ...
        viewer_version = importlib.metadata.version("lima-viewer")
        ...
        assert sdk_major == viewer_major, (
            f"Major version mismatch: rerun-sdk={sdk_version}, lima-viewer={viewer_version}. "
            f"These are likely incompatible."
        )
        assert abs(sdk_minor - viewer_minor) <= 1, (
            f"Version drift too large: rerun-sdk={sdk_version}, lima-viewer={viewer_version}. "
            f"Update lima-viewer to match rerun-sdk or vice versa."
        )
