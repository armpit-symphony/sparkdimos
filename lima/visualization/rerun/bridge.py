...
from lima.protocol.pubsub.patterns import Glob, pattern_matches
...
if TYPE_CHECKING:
    ...
    from lima.protocol.pubsub.spec import SubscribeAllCapable
        ...
        if self.config.viewer_mode == "native":
            try:
                ...
                rerun_bindings.spawn(
                    ...
                    executable_name="lima-viewer",
                    memory_limit=self.config.memory_limit,
                )
            except ImportError:
                pass  # lima-viewer not installed
            except Exception:
                logger.warning(
                    "lima-viewer found but failed to spawn, falling back to stock rerun",
                    exc_info=True,
                )
