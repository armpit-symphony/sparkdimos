...
Quick start:
    ...
    lima run xarm6-planner-only

    # 2. Keyboard teleop with mock arm:
    lima run keyboard-teleop-xarm7

    # 3. Interactive RPC client (plan, preview, execute from Python):
    lima run xarm7-planner-coordinator
    python -i -m lima.manipulation.planning.examples.manipulation_client
"""
...
from lima.manipulation.pick_and_place_module import pick_and_place_module
from lima.manipulation.planning.spec import RobotModelConfig
from lima.msgs.geometry_msgs import PoseStamped, Quaternion, Transform, Vector3
...
from lima.robot.foxglove_bridge import foxglove_bridge  # TODO: migrate to rerun
...
# Usage: lima run xarm7-planner-coordinator
...
# Usage: lima run coordinator-mock, then lima run xarm7-planner-coordinator-agent
...
# Usage: lima run coordinator-mock, then lima run xarm-perception
...
# Usage: lima run coordinator-mock, then lima run xarm-perception-agent
