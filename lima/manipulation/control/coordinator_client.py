...
Usage:
    ...
    lima run coordinator-mock          # Single arm
    lima run coordinator-dual-mock     # Dual arm

    # Terminal 2: Run this client
    python -m lima.manipulation.control.coordinator_client
    python -m lima.manipulation.control.coordinator_client --task traj_left
    python -m lima.manipulation.control.coordinator_client --task traj_right
...
from lima.control.coordinator import ControlCoordinator
from lima.core.rpc_client import RPCClient
from lima.manipulation.planning.trajectory_generator.joint_trajectory_generator import (
    JointTrajectoryGenerator,
)

if TYPE_CHECKING:
    from lima.msgs.trajectory_msgs import JointTrajectory
...
def _run_client(client: CoordinatorClient, task: str, vel: float, accel: float) -> int:
    ...
    try:
        ...
        if not hardware:
            ...
            print("Start with: lima run coordinator-mock")
        ...
        print(f"\nConnection error: {e}")
        print("Make sure coordinator is running: lima run coordinator-mock")
        return 1
