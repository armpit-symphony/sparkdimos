...
from lima.constants import LIMA_LOG_DIR, LIMA_PROJECT_ROOT
...
def set_run_log_dir(log_dir: str | Path) -> None:
    ...
    os.environ["LIMA_RUN_LOG_DIR"] = str(log_dir)
...
def _get_log_directory() -> Path:
    ...
    if (LIMA_PROJECT_ROOT / ".git").exists():
        ...
        if xdg_state_home:
            log_dir = Path(xdg_state_home) / "lima" / "logs"
        else:
            log_dir = Path.home() / ".local" / "state" / "lima" / "logs"

    try:
        log_dir.mkdir(parents=True, exist_ok=True)
    except (PermissionError, OSError):
        log_dir = Path(tempfile.gettempdir()) / "lima" / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
...
def _get_log_file_path() -> Path:
    ...
    env_log_dir = os.environ.get("LIMA_RUN_LOG_DIR")
    if env_log_dir:
        p = Path(env_log_dir)
    ...
    pid = os.getpid()
    return log_dir / f"lima_{timestamp}_{pid}.jsonl"
...
def setup_logger(*, level: int | None = None) -> Any:
    ...
    try:
        name = str(Path(name).relative_to(LIMA_PROJECT_ROOT))
    ...
    if level is None:
        level_name = os.getenv("LIMA_LOG_LEVEL", "INFO")
