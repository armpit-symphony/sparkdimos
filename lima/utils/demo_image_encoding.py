...
Run it with uncompressed LCM:
    ...
    python lima/utils/demo_image_encoding.py

Run it with JPEG LCM:

    python lima/utils/demo_image_encoding.py --use-jpeg
"""
...
from lima.core.transport import JpegLcmTransport, LCMTransport
from lima.msgs.sensor_msgs import Image
from lima.robot.foxglove_bridge import FoxgloveBridge
from lima.utils.fast_image_generator import random_image
...
def main() -> None:
    ...
    lima = ModuleCoordinator(n=2)
    lima.start()
    emitter = lima.deploy(EmitterModule)
    receiver = lima.deploy(ReceiverModule)
    ...
    try:
        ...
        lima.close()  # type: ignore[attr-defined]
