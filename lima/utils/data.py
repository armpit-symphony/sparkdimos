...
def _get_user_data_dir() -> Path:
    ...
    if os.environ.get("VIRTUAL_ENV"):
        venv_data_dir = Path(
            f"{os.environ.get('VIRTUAL_ENV')}/lib/python{sys.version_info.major}.{sys.version_info.minor}/site-packages/lima/data"
        )
    ...
    if system == "Linux":
        ...
        if xdg_data_home:
            return Path(xdg_data_home) / "lima"
        return Path.home() / ".local" / "share" / "lima"
    elif system == "Darwin":  # macOS
        return Path.home() / "Library" / "Application Support" / "lima"
    else:
...
def _get_repo_root() -> Path:
    ...
    if (LIMA_PROJECT_ROOT / ".git").exists():
        return LIMA_PROJECT_ROOT
    ...
    try:
        ...
        data_dir = Path(tempfile.gettempdir()) / "lima"
        data_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "--branch",
                    "main",
                    "https://github.com/armpit-symphony/lima-robotics.git",
                    str(repo_dir),
                ],
                check=True,
                capture_output=True,
            ...
            raise RuntimeError(
                f"Failed to clone lima repository: {e.stderr}\n"
                f"Make sure you can access https://github.com/armpit-symphony/lima-robotics.git"
            )
