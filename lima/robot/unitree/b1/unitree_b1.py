...
from lima.core.resource import Resource
from lima.core.transport import LCMTransport, ROSTransport
from lima.msgs.geometry_msgs import PoseStamped, TwistStamped
from lima.msgs.nav_msgs.Odometry import Odometry
from lima.msgs.std_msgs import Int32
from lima.msgs.tf2_msgs.TFMessage import TFMessage
from lima.robot.robot import Robot
from lima.robot.unitree.b1.connection import (
    B1ConnectionModule,
    MockB1ConnectionModule,
)
from lima.skills.skills import SkillLibrary
from lima.types.robot_capabilities import RobotCapability
...
class UnitreeB1(Robot, Resource):
    ...
    def __init__(
        ...
        self.joystick = None
        self._lima = ModuleCoordinator(n=2)

        os.makedirs(self.output_dir, exist_ok=True)
        logger.info(f"Robot outputs will be saved to: {self.output_dir}")

    def start(self) -> None:
        """Start the B1 robot - initialize LIMA, deploy modules, and start them."""

        logger.info("Initializing LIMA...")
        self._lima.start()

        logger.info("Deploying connection module...")
        if self.test_mode:
            self.connection = self._lima.deploy(MockB1ConnectionModule, self.ip, self.port)  # type: ignore[assignment]
        else:
            self.connection = self._lima.deploy(B1ConnectionModule, self.ip, self.port)  # type: ignore[assignment]
        ...
        if self.enable_joystick:
            from lima.robot.unitree.b1.joystick_module import JoystickModule

            self.joystick = self._lima.deploy(JoystickModule)  # type: ignore[assignment]
