...
LIMA --replay run drone-basic

# Agentic mode with replay
LIMA --replay run drone-agentic

# Real drone — indoor (velocity-based odometry)
LIMA run drone-basic

# Real drone — outdoor (GPS-based odometry)
LIMA run drone-basic --set outdoor=true

# Agentic with LLM control
LIMA run drone-agentic
```

To interact with the agent, run `LIMA humancli` in a separate terminal.
...
RosettaDrone is an Android app that bridges DJI SDK to MAVLink protocol. Without it, the drone cannot communicate with LIMA.
...
### Hardware Connection
```
DJI Drone ← Wireless → DJI Controller ← USB → Android Device ← WiFi → LIMA Computer
```
...
- Run `LIMA humancli` to send commands
