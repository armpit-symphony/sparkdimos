...
from lima.robot.drone.connection_module import DroneConnectionModule
from lima.robot.drone.dji_video_stream import FakeDJIVideoStream

# Drone class removed - use blueprints instead
from lima.robot.drone.mavlink_connection import FakeMavlinkConnection, MavlinkConnection
...
class TestReplayMode(unittest.TestCase):
    ...
    def test_fake_mavlink_connection(self) -> None:
        ...
        with patch("lima.utils.testing.TimedSensorReplay") as mock_replay:
    ...
    def test_connection_module_replay_mode(self) -> None:
        ...
        with patch("lima.robot.drone.mavlink_connection.FakeMavlinkConnection") as mock_fake_conn:
            with patch("lima.robot.drone.dji_video_stream.FakeDJIVideoStream") as mock_fake_video:
...
class TestDroneFullIntegration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Mock the LIMA core module
        self.mock_lima = MagicMock()
        self.mock_lima.deploy.return_value = MagicMock()

        # Mock pubsub.lcm.autoconf
        self.pubsub_patch = patch("lima.protocol.pubsub.lcm.autoconf")
        self.pubsub_patch.start()

        # Mock FoxgloveBridge
        self.foxglove_patch = patch("lima.robot.drone.drone.FoxgloveBridge")
    ...
    @patch("lima.robot.drone.drone.ModuleCoordinator")
    @patch("lima.utils.testing.TimedSensorReplay")
    def test_full_system_with_replay(self, mock_replay, mock_coordinator_class) -> None:
        ...
        mock_coordinator_class.return_value = self.mock_lima
        ...
        self.mock_lima.deploy.side_effect = deploy_side_effect

        # Start the drone system
        drone.start()

        # Verify modules were deployed
        self.assertEqual(self.mock_lima.deploy.call_count, 4)
        ...
        self.mock_lima.stop.assert_called_once()
...
class TestDroneControlCommands(unittest.TestCase):
    ...
    @patch("lima.utils.testing.TimedSensorReplay")
    @patch("lima.utils.data.get_data")
...
class TestVisualServoingEdgeCases(unittest.TestCase):
    ...
    def test_output_clamping(self) -> None:
        ...
        from lima.robot.drone.drone_visual_servoing_controller import (
