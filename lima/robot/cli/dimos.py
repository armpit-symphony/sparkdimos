...
from lima.agents.mcp.mcp_adapter import McpAdapter, McpError
from lima.core.global_config import GlobalConfig, global_config
from lima.core.run_registry import get_most_recent, is_pid_alive, stop_entry
...
def run(
    ...
    logger.info("Starting LIMA")

    from lima.core.blueprints import autoconnect
    from lima.core.run_registry import (
        LOG_BASE_DIR,
        RunEntry,
    ...
    from lima.robot.get_all_blueprints import get_by_name, get_module_by_name
    from lima.utils.logging_config import set_run_log_dir, setup_exception_handler
    ...
    # Workers inherit LIMA_RUN_LOG_DIR env var via forkserver.
    ...
    if daemon:
        ...
        typer.echo("✓ LIMA running in background\n")
        typer.echo(f"  Run ID:    {run_id}")
        typer.echo(f"  Log:       {log_dir}")
        typer.echo("  Stop:      lima stop")
        typer.echo("  Status:    lima status")
...
def status() -> None:
    ...
    if not entry:
        typer.echo("No running LIMA instance")
        return
...
def stop(
    ...
    entry = get_most_recent(alive_only=True)
    if not entry:
        typer.echo("No running LIMA instance", err=True)
        raise typer.Exit(1)
...
def log_cmd(
    ...
    from lima.core.log_viewer import follow_log, format_line, read_log, resolve_log_path
...
def _get_adapter() -> McpAdapter:
    ...
    from lima.agents.mcp.mcp_adapter import McpAdapter
...
def mcp_list_tools() -> None:
    ...
    try:
        ...
        typer.echo("Error: no running MCP server (is LIMA running?)", err=True)
        raise typer.Exit(1)
...
def agent_send_cmd(
    ...
    """Send a message to the running LIMA agent via MCP."""
    try:
...
def restart(
    ...
    """Restart the running LIMA instance with the same arguments."""
    entry = get_most_recent(alive_only=True)
    if not entry:
        typer.echo("No running LIMA instance to restart", err=True)
        raise typer.Exit(1)
...
def list_blueprints() -> None:
    ...
    from lima.robot.all_blueprints import all_blueprints
...
def lcmspy(ctx: typer.Context) -> None:
    ...
    from lima.utils.cli.lcmspy.run_lcmspy import main as lcmspy_main
...
def agentspy(ctx: typer.Context) -> None:
    ...
    from lima.utils.cli.agentspy.agentspy import main as agentspy_main
...
def humancli(ctx: typer.Context) -> None:
    ...
    from lima.utils.cli.human.humanclianim import main as humancli_main
...
def top(ctx: typer.Context) -> None:
    ...
    from lima.utils.cli.dtop import main as dtop_main
...
def echo(
    ...
    from lima.robot.cli.topic import topic_echo
...
def send(
    ...
    from lima.robot.cli.topic import topic_send
...
def rerun_bridge_cmd(
    ...
    from lima.visualization.rerun.bridge import run_bridge
