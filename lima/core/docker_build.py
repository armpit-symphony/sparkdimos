...
Converts any Dockerfile into a LIMA module container by appending a footer
that installs LIMA and creates the module entrypoint.
"""
...
if TYPE_CHECKING:
    ...
    from lima.core.docker_runner import DockerModuleConfig
...
# Footer appended to Dockerfiles for LIMA module conversion
LIMA_FOOTER = f"""
# ==== {DIMOS_SENTINEL} ====
# Copy LIMA source from build context
COPY lima /lima/source/lima/
COPY pyproject.toml /lima/source/
COPY docker/python/module-install.sh /tmp/module-install.sh

# Install LIMA and create entrypoint
RUN bash /tmp/module-install.sh /lima/source && rm /tmp/module-install.sh
...
def _convert_dockerfile(dockerfile: Path) -> Path:
    """Append LIMA footer to Dockerfile. Returns path to converted file."""
    ...
    logger.info(f"Converting {dockerfile.name} to LIMA format")

    converted = dockerfile.parent / f".{dockerfile.name}.lima"
