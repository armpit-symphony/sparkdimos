...
Simulates what happens when LIMA crashes and restarts:
...
Usage:
    python -m lima.core.tests.e2e_mcp_killtest
    python -m lima.core.tests.e2e_mcp_killtest --cycles 5
"""
...
def test_mcp_basic_ops() -> int:
    ...
    # lima/status
    try:
        result = mcp_call("lima/status")
        assert "StressTestModule" in result["result"]["modules"]
        p(f"lima/status \u2192 pid={result['result']['pid']}")
    except Exception as e:
        p(f"lima/status failed: {e}", ok=False)
        failures += 1

    # lima/agent_send
    try:
        result = mcp_call("lima/agent_send", {"message": "hello from killtest"})
...
def run_kill_restart_cycle(cycle: int) -> int:
    failures = 0
    section(f"CYCLE {cycle}: Starting LIMA")
    ...
    # RESTART
    section(f"CYCLE {cycle}: Restarting LIMA")
    ready2 = ctx.Event()
