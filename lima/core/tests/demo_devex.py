...
Simulates the complete LIMA developer workflow as if an OpenClaw agent
is using LIMA for the first time:

1. lima run stress-test --daemon  (start in background)
2. lima status                    (verify running)
3. lima mcp list-tools            (discover tools)
4. lima mcp call echo             (call a tool)
5. lima mcp status                (module info)
6. lima mcp modules               (module-skill mapping)
7. lima agent-send "hello"        (send to agent)
8. Check logs for responses
9. lima stop                     (clean shutdown)
10. lima status                   (verify stopped)

Usage:
    python -m lima.core.tests.e2e_devex_test
"""
...
def run_lima(*args: str, timeout: float = 30) -> subprocess.CompletedProcess[str]:
    """Run a lima CLI command."""
    cmd = [VENV_PYTHON, "-m", "lima.robot.cli.dimos", *args]
...
def main() -> None:
    ...
    print("  Simulating: OpenClaw agent using LIMA")
    print("=" * 60)

    # ---------------------------------------------------------------
    # Step 1: lima run stress-test --daemon
    # ---------------------------------------------------------------
    section("Step 1: lima run stress-test --daemon")
    result = run_lima("run", "stress-test", "--daemon", timeout=60)
    ...
    # Step 2: lima status
    # ---------------------------------------------------------------
    section("Step 2: lima status")
    result = run_lima("status")
    ...
    # ---------------------------------------------------------------
    # Step 3: lima mcp list-tools
    # ---------------------------------------------------------------
    section("Step 3: lima mcp list-tools")
    result = run_lima("mcp", "list-tools")
    if result.returncode == 0:
        try:
    ...
    # ---------------------------------------------------------------
    # Step 4: lima mcp call echo --arg message=hello
    # ---------------------------------------------------------------
    section("Step 4: lima mcp call echo --arg message=hello")
    result = run_lima("mcp", "call", "echo", "--arg", "message=hello-from-devex-test")
    ...
    # ---------------------------------------------------------------
    # Step 5: lima mcp status
    # ---------------------------------------------------------------
    section("Step 5: lima mcp status")
    result = run_lima("mcp", "status")
    if result.returncode == 0:
        try:
    ...
    # ---------------------------------------------------------------
    # Step 6: lima mcp modules
    # ---------------------------------------------------------------
    section("Step 6: lima mcp modules")
    result = run_lima("mcp", "modules")
    if result.returncode == 0:
        try:
            data = json.loads(result.stdout)
    ...
    # ---------------------------------------------------------------
    # Step 7: lima agent-send "hello"
    # ---------------------------------------------------------------
    section("Step 7: lima agent-send 'what tools do you have?'")
    result = run_lima("agent-send", "what tools do you have?")
    if result.returncode == 0:
    ...
    log_base = os.path.expanduser("~/.local/state/lima/logs")
    ...
    # Step 9: lima stop
    # ---------------------------------------------------------------
    section("Step 9: lima stop")
    result = run_lima("stop")
    ...
    # ---------------------------------------------------------------
    # Step 10: lima status (verify stopped)
    # ---------------------------------------------------------------
    section("Step 10: lima status (verify stopped)")
    result = run_lima("status")
    print(f"  output: {result.stdout.strip()[:200]}")
    if (
        "no running" in result.stdout.lower()
        or "no lima" in result.stdout.lower()
