...
# Registry tests
# ---------------------------------------------------------------------------
from lima.core import run_registry
...
def tmp_registry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("lima.core.run_registry.REGISTRY_DIR", tmp_path)
    return tmp_path
...
class TestRunEntryCRUD:
    ...
    def test_save_creates_directory(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr("lima.core.run_registry.REGISTRY_DIR", nested)
        entry = _make_entry()
        entry.save()
...
from lima.core.daemon import daemonize, install_signal_handlers
...
class TestStatusCommand:
    """Tests for `lima status` CLI command."""
...
class TestStopCommand:
    """Tests for `lima stop` CLI command."""
    ...
    def test_stop_sends_sigterm(self, tmp_path, monkeypatch):
        ...
        from lima.core.run_registry import stop_entry
    ...
    def test_get_most_recent_returns_latest(self, tmp_path, monkeypatch):
        ...
        from lima.core.run_registry import get_most_recent
