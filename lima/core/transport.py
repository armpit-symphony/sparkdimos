...
from lima.core.stream import In, Out, Stream, Transport
from lima.msgs.protocol import LimaMsg
from lima.utils import colors
...
from lima.protocol.pubsub.impl.jpeg_shm import JpegSharedMemory
from lima.protocol.pubsub.impl.lcmpubsub import LCM, JpegLCM, PickleLCM, Topic as LCMTopic
from lima.protocol.pubsub.impl.rospubsub import limaROS, ROSTopic
from lima.protocol.pubsub.impl.shmpubsub import BytesSharedMemory, PickleSharedMemory
...
class ROSTransport(PubSubTransport[LimaMsg]):
    _ros: limaROS | None = None

    def __init__(self, topic: str, msg_type: type[LimaMsg], **kwargs: Any) -> None:
    ...
    def broadcast(self, _: Out[LimaMsg], msg: LimaMsg) -> None:
    ...
    def subscribe(
        self, callback: Callable[[LimaMsg], Any], selfstream: Stream[LimaMsg] | None = None
    ) -> Callable[[], None]:
        if self._ros is None:
            self.start()
            assert self._ros is not None  # for type narrowing
    ...
    def start(self) -> None:
        if self._ros is None:
            self._ros = limaROS(**self._kwargs)
            self._ros.start()
...
if DDS_AVAILABLE:
    from lima.protocol.pubsub.impl.ddspubsub import DDS, Topic as DDSTopic
