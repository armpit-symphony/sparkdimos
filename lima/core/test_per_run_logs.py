...
from lima.utils import logging_config
from lima.utils.logging_config import _get_log_file_path, get_run_log_dir, set_run_log_dir


@pytest.fixture(autouse=True)
def _clean_env(monkeypatch):
    """Remove LIMA_RUN_LOG_DIR from env and reset module globals between tests."""
    monkeypatch.delenv("LIMA_RUN_LOG_DIR", raising=False)
...
class TestSetRunLogDir:
    ...
    def test_sets_env_var(self, tmp_path):
        ...
        set_run_log_dir(log_dir)
        assert os.environ["LIMA_RUN_LOG_DIR"] == str(log_dir)
...
class TestLogFilePathRouting:
    ...
    def test_routes_via_env_var(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LIMA_RUN_LOG_DIR", str(env_dir))
    ...
    def test_falls_back_to_legacy(self):
        path = logging_config._get_log_file_path()
        assert path.name.startswith("lima_")
