...
"""Run registry for tracking LIMA daemon processes."""
...
def _get_state_dir() -> Path:
    """XDG_STATE_HOME compliant state directory for lima."""
    xdg = os.environ.get("XDG_STATE_HOME")
    if xdg:
        return Path(xdg) / "lima"
    return Path.home() / ".local" / "state" / "lima"
...
@dataclass
class RunEntry:
    """Metadata for a single LIMA run (daemon or foreground)."""
...
def stop_entry(entry: RunEntry, force: bool = False) -> tuple[str, bool]:
    """Stop a LIMA instance by registry entry.
