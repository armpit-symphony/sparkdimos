...
from lima.core.testing import MockRobotClient
...
@pytest.mark.slow
def test_*(lima, subscriber_class) -> None:
    robot = lima.deploy(MockRobotClient)
    ...
    subscriber = lima.deploy(subscriber_class)
...
def test_get_next(lima) -> None:
    ...
    subscriber = lima.deploy(RXPYSubscriber)
    subscriber.odom.connect(robot.odometry)
