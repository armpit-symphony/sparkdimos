...
from lima.core.worker_manager import WorkerManager
from lima.utils.logging_config import setup_logger

if TYPE_CHECKING:
    from lima.core.module import Module, ModuleT
    from lima.core.resource_monitor.monitor import StatsMonitor
    from lima.core.rpc_client import ModuleProxy
    from lima.core.worker import Worker
...
class ModuleCoordinator(Resource):  # type: ignore[misc]
    ...
    def deploy(self, module_class: type[ModuleT], *args, **kwargs) -> ModuleProxy:  # type: ignore[no-untyped-def]
        if not self._client:
            raise ValueError("Trying to lima.deploy before the client has started")
