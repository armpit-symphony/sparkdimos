...
from lima.core.docker_build import build_image, image_exists
from lima.core.module import Module, ModuleConfig
from lima.core.rpc_client import RpcCall
from lima.protocol.rpc import LCMRPC
from lima.utils.logging_config import setup_logger
from lima.visualization.rerun.bridge import RERUN_GRPC_PORT, RERUN_WEB_PORT
...
class DockerModuleConfig(ModuleConfig):
    """
    Configuration for running a LIMA module inside Docker.
...
class DockerModule:
    ...
    def __init__(self, module_class: type[Module], *args: Any, **kwargs: Any) -> None:
        ...
        self._container_name = (
            ...
            or f"lima_{module_class.__name__.lower()}_{os.getpid()}_{int(time.time())}"
        )
    ...
    def _add_label_args(self, cmd: list[str], cfg: DockerModuleConfig) -> None:
        """Add --label args with LIMA defaults."""
        labels = dict(cfg.docker_labels)
        labels.setdefault("lima.kind", "module")
        labels.setdefault("lima.module", self._module_class.__name__)
    ...
    def _build_container_command(self, cfg: DockerModuleConfig) -> list[str]:
        ...
        # LIMA base image entrypoint already runs "lima.core.docker_runner run"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="lima.core.docker_runner")
